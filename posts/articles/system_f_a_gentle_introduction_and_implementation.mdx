...
description: 'Let's understand a little bit about types'
date: '2023-03-02'
tags: ['functional-programming', 'type-theory']
---

System F is a Typed Lambda Calculus that incorporates a mechanism of universal quantification over types. Therefore, to better understand the concepts discussed in this post, it is assumed that you already have a basic understanding of the ([Simply Typed](https://en.wikipedia.org/wiki/Simply_typed_lambda_calculus)) [Lambda Calculus](https://en.wikipedia.org/wiki/Lambda_calculus) and the Lambda Calculus itself, along with concepts such as bound and free variables, alpha-equivalence, and capture-avoiding substitution.

## Introduction

In System F, the [universal quantifier](https://en.wikipedia.org/wiki/Universal_quantification) over types enables the creation of functions that can be applied to any data type, allowing for greater code reusability and flexibility. This is particularly useful in situations where a function needs to be applied to a variety of different data types.
